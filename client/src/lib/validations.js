...
export const validateInt = (val, precision) => {
  ...
  else if (parseInt(val) > Number.MAX_SAFE_INTEGER || parseInt(val) < Number.MIN_SAFE_INTEGER)
    return [false, '-1']
...
export const validateFloat = (val, precision) => {
  ...
  if (parseInt(val) > Number.MAX_SAFE_INTEGER || parseInt(val) < Number.MIN_SAFE_INTEGER)
    return [false, '-1']
